    ...
    <div class="card mb-4">
      <div class="card-body">
          <div class="col-lg-6">
            ...
            <p class="card-text"><label>재능 분류 : </label>${list.tals_Kinds} | ${list.tals_Kinds_2}</p>
            <p class="card-text"><label>가격 : </label><fmt:formatNumber value="${list.tals_Price}" pattern="###,###,###원"/></p>
            <p class="card-text" style ="position:relative; left : 0px;"><div id="btn-place"><a href="/talent/talent_S_view?n=${list.tals_Code}"  class="btn btn-primary">상세보기 &rarr;</a></div> </p> 
          </div>
      </div>
    </div>
...
<div id="page">
 <ul>
  <c:if test="${pageMaker.prev}">
   <li><a href="talent_S_list_2${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
  </c:if> 
  
  <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
   <li><a href="talent_S_list_2${pageMaker.makeQuery(idx)}">${idx}</a></li>
  </c:forEach>
    
  <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
   <li><a href="talent_S_list_2${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
  </c:if> 
 </ul>
</div>
  <!-- Footer -->
